...
{
  // we release all process resources (Prosses Memory Layout)
...
/**
   retrieves if existing the process memory layout of pid (process list contain Process Memory Layouts)
*/
                               ...
                               uint32_t limit)
{
  ...
  // we need to check that there is enough free physical memory for the new process
  ...
  // we select frame from free frame_list 
  ...
  for (uint32_t p = base; p < base + limit; ++p)
  {
    ...
    // assign the pid to the frame
    m.frame_to_pid[frame_num] = pmem->pid;

    // we move the memory in the process region, move free frame in segment table
    List_detach(&m.frame_list, m.frame_list.first);
