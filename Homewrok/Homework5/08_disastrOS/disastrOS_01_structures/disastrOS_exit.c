...
void internal_exit(){
  ...
  while(running->children.first){
    ...
    PCB* pcb=pcb_ptr->pcb;
    pcb->signals |= (DSOS_SIGHUP & pcb->signals_mask); //setting the second bit to one to communicate a signal for children
  }

  //atempt someone call its return value
  running->status=Zombie;
  List_insert(&zombie_list, zombie_list.last, (ListItem*) running); //adding on the Zombie's list
  running->parent->signals |= (DSOS_SIGCHLD & running->parent->signals_mask); //setting the second bit to one to communicate a signal for parent
  ...
  // since he called  wait or waitpid, we wake him up
	  ...
	  running->parent->syscall_args[0]==running->pid) //atempt the children, i made a wait with the running's pid like args (waitpid(running->pid))
      ){
    PCB* parent= (PCB*) List_detach(&waiting_list, (ListItem*) running->parent);
    assert(parent);
    parent->status=Running; //take the place of the child
    ...
    parent->syscall_retvalue=running->pid; //first return vale is the child pid 
    int* result=(int*)parent->syscall_args[1]; //put the return value of the child in the adress passed on parent->syscall_args[1]
    if (result)
