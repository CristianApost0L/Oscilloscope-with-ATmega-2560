...
typedef struct GlobalMemoryLayout{
  int frame_to_pid[PAGES_NUM];   // list of frame's pid (page's pid is the same, one page <-> one frame)
  ListHead frame_list;           // list of free frame (real memory adresses)
  ListHead process_list;         // list of process memory layout
} GlobalMemoryLayout;
