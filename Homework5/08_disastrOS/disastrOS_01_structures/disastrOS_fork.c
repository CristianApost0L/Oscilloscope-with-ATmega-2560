...
void internal_fork() {
  ...
  new_pcb=PCB_alloc();
  if (!new_pcb) {
    running->syscall_retvalue=DSOS_ESPAWN; //the fork return an error 
    return;
  } 
  ...
  // adds a pointer to the new process to the children list of running, ONLY THE POINTER
  PCBPtr* new_pcb_ptr=PCBPtr_alloc(new_pcb);
  assert(new_pcb_ptr);
  List_insert(&running->children, running->children.last, (ListItem*) new_pcb_ptr);

  //adds the new process to the ready queue, HERE THE PROCESS
