...
//Necessario per rendere le syscall con un numero variabile di argomenti
int disastrOS_syscall(int syscall_num, ...) {
  ...
  //Simula la recezione di una trap per portare all'esecuzione dell'handler associato, ovvero la syscall richiesta
  disastrOS_trap();
